...
// 提款(將錢從測試帳號提出)Request
type MoneyOutRequest struct {
...
//	@Summary		提款(將錢從測試帳號提出)
//	@Tags			接入輔助工具
//	@Description	account 與 amount 需要是正確的才能從測試帳號提款
//	@Description	{相對應站點的URL}/peace/money/out
...
//	@Success		200			{object}	MoneyOutResponse
//	@Router			/peace/money/out [post]
//
//	@Security		Bearer
func MoneyOut(traceCode, requestTime string, r *http.Request) []byte {
	//#region 讀取request
	...
	if err != nil {
		zaplog.Errorw(innertrace.ParseFormError, innertrace.FunctionNode, cid.MoneyOut, innertrace.TraceNode, traceCode, innertrace.DataNode, err)
	...
	//log request model
	zaplog.Infow(innertrace.LogRequestModel, innertrace.FunctionNode, cid.MoneyOut, innertrace.TraceNode, traceCode, innertrace.DataNode, tracer.MergeMessage("request", string(serializer.JsonMarshal(traceCode, request))))
	...
	//轉發 CQ server
	_, cq9Data, isOK := cq9.MoneyOut(traceCode, request.Account, request.Amount, request.Currency)
	//轉發失敗輸出錯誤
	if !isOK {
		errResp := responseError(errorcode.InnerError, requestTime, traceCode, "forwarded MoneyOut failure!")
		return errResp
	}

	//轉換data
	data := MoneyOutResponse{
		Data: MoneyOutResponseData(cq9Data.Data),
